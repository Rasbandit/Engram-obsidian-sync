...
var require_diff_match_patch = __commonJS({
  "node_modules/diff-match-patch/index.js"(exports, module2) {
    ...
    diff_match_patch.prototype.diff_main = function(text1, text2, opt_checklines, opt_deadline) {
      ...
      if (text1 == text2)
        return text1 ? [new diff_match_patch.Diff(DIFF_EQUAL, text1)] : [];
      ...
      return commonprefix && diffs.unshift(new diff_match_patch.Diff(DIFF_EQUAL, commonprefix)), commonsuffix && diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, commonsuffix)), this.diff_cleanupMerge(diffs), diffs;
    };
    ...
    diff_match_patch.prototype.diff_lineMode_ = function(text1, text2, deadline) {
      ...
      this.diff_charsToLines_(diffs, linearray), this.diff_cleanupSemantic(diffs), diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, ""));
...
`, lineStart), lineEnd == -1 && (lineEnd = text.length - 1);
    ...
    diff_match_patch.prototype.diff_cleanupSemantic = function(diffs) {
      ...
      for (changes && this.diff_cleanupMerge(diffs), this.diff_cleanupSemanticLossless(diffs), pointer = 1; pointer < diffs.length; ) {
        if (diffs[pointer - 1][0] == DIFF_DELETE && diffs[pointer][0] == DIFF_INSERT) {
          ...
          overlap_length1 >= overlap_length2 ? (overlap_length1 >= deletion.length / 2 || overlap_length1 >= insertion.length / 2) && (diffs.splice(pointer, 0, new diff_match_patch.Diff(
            DIFF_EQUAL,
            insertion.substring(0, overlap_length1)
          )), diffs[pointer - 1][1] = deletion.substring(0, deletion.length - overlap_length1), diffs[pointer + 1][1] = insertion.substring(overlap_length1), pointer++) : (overlap_length2 >= deletion.length / 2 || overlap_length2 >= insertion.length / 2) && (diffs.splice(pointer, 0, new diff_match_patch.Diff(
            DIFF_EQUAL,
      ...
      for (var diffs = [], diffsLength = 0, pointer = 0, tokens = delta.split(/\t/g), x = 0; x < tokens.length; x++) {
        ...
        switch (tokens[x].charAt(0)) {
          ...
          case "=":
            ...
            tokens[x].charAt(0) == "=" ? diffs[diffsLength++] = new diff_match_patch.Diff(DIFF_EQUAL, text) : diffs[diffsLength++] = new diff_match_patch.Diff(DIFF_DELETE, text);
            break;
          default:
            if (tokens[x])
    ...
    diff_match_patch.prototype.patch_addContext_ = function(patch, text) {
      if (text.length != 0) {
        ...
        prefix && patch.diffs.unshift(new diff_match_patch.Diff(DIFF_EQUAL, prefix));
        var suffix = text.substring(
          patch.start2 + patch.length1,
          patch.start2 + patch.length1 + padding
        );
        suffix && patch.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, suffix)), patch.start1 -= prefix.length, patch.start2 -= prefix.length, patch.length1 += prefix.length + suffix.length, patch.length2 += prefix.length + suffix.length;
      }
    };
    ...
    diff_match_patch.prototype.patch_addPadding = function(patches) {
      ...
      if (diffs.length == 0 || diffs[0][0] != DIFF_EQUAL)
        diffs.unshift(new diff_match_patch.Diff(DIFF_EQUAL, nullPadding)), patch.start1 -= paddingLength, patch.start2 -= paddingLength, patch.length1 += paddingLength, patch.length2 += paddingLength;
      ...
      if (patch = patches[patches.length - 1], diffs = patch.diffs, diffs.length == 0 || diffs[diffs.length - 1][0] != DIFF_EQUAL)
        diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, nullPadding)), patch.length1 += paddingLength, patch.length2 += paddingLength;
      ...
      for (var patch_size = this.Match_MaxBits, x = 0; x < patches.length; x++)
        if (!(patches[x].length1 <= patch_size)) {
          ...
          for (var start1 = bigpatch.start1, start2 = bigpatch.start2, precontext = ""; bigpatch.diffs.length !== 0; ) {
            ...
            for (patch.start1 = start1 - precontext.length, patch.start2 = start2 - precontext.length, precontext !== "" && (patch.length1 = patch.length2 = precontext.length, patch.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, precontext))); bigpatch.diffs.length !== 0 && patch.length1 < patch_size - this.Patch_Margin; ) {
              var diff_type = bigpatch.diffs[0][0], diff_text = bigpatch.diffs[0][1];
              diff_type === DIFF_INSERT ? (patch.length2 += diff_text.length, start2 += diff_text.length, patch.diffs.push(bigpatch.diffs.shift()), empty = !1) : diff_type === DIFF_DELETE && patch.diffs.length == 1 && patch.diffs[0][0] == DIFF_EQUAL && diff_text.length > 2 * patch_size ? (patch.length1 += diff_text.length, start1 += diff_text.length, empty = !1, patch.diffs.push(new diff_match_patch.Diff(diff_type, diff_text)), bigpatch.diffs.shift()) : (diff_text = diff_text.substring(
                0,
            ...
            postcontext !== "" && (patch.length1 += postcontext.length, patch.length2 += postcontext.length, patch.diffs.length !== 0 && patch.diffs[patch.diffs.length - 1][0] === DIFF_EQUAL ? patch.diffs[patch.diffs.length - 1][1] += postcontext : patch.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, postcontext))), empty || patches.splice(++x, 0, patch);
          }
        }
    };
...
function renderAccountTab(ctx) {
  var _a;
  let { containerEl, app, plugin, redisplay, startDeviceFlow, openProgressModal } = ctx, isOAuth = !!plugin.settings.refreshToken, hasApiKey = !!plugin.settings.apiKey;
  ...
  new import_obsidian9.Setting(containerEl).setName("Connection").setHeading(), new import_obsidian9.Setting(containerEl).setName("Engram URL").setDesc("Full URL to your Engram instance (e.g. http://10.0.20.214:8000)").addText(
  ...
  ), (plugin.settings.apiKey || plugin.settings.refreshToken) && (new import_obsidian9.Setting(containerEl).setName("Vault").setHeading(), new import_obsidian9.Setting(containerEl).setName("Sync vault").setDesc("Select which vault this plugin syncs with").addDropdown((dropdown) => {
  ...
  })), new import_obsidian9.Setting(containerEl).setName("Actions").setHeading(), new import_obsidian9.Setting(containerEl).setName("Sync now").setDesc("Pull remote changes and push local changes").addButton(
    (btn) => btn.setButtonText("Sync").onClick(async () => {
      ...
      try {
        ...
        let progressModal = await openProgressModal(), { pulled, pushed } = await plugin.syncEngine.fullSync(), errors = (_b = (_a2 = plugin.syncEngine.syncLog) == null ? void 0 : _a2.errorCount()) != null ? _b : 0;
...
var DiffMatchPatch = require_diff_match_patch(), dmp = new DiffMatchPatch();
